...
class FlexmarkNodeTreeVisitor(
    ...
    private fun highlightIndentedCodeBlock(indentedCodeBlock: IndentedCodeBlock) {
        ...
        writer.add(
            spanPool.indentedCodeBlock(markwonTheme),
            lineStartOffset,
            indentedCodeBlock.endOffset
        )
        writer.add(
            spanPool.monospaceTypeface(),
            indentedCodeBlock.startOffset,
            indentedCodeBlock.endOffset
        )
    }
    ...
    private fun highlightBlockQuote(blockQuote: BlockQuote) {
        ...
        writer.add(
            spanPool.foregroundColor(syntaxColor),
            markerStartOffset,
            blockQuote.startOffset + 1
        )

        // Text color.
        writer.add(
            spanPool.foregroundColor(blockQuoteTextColor),
            blockQuote.startOffset - nestedParents,
            blockQuote.endOffset
        )
    }

    private fun highlightHeading(heading: Heading) {
        writer.add(
            spanPool.heading(heading.level, markwonTheme),
            heading.startOffset,
            heading.endOffset
        )
        writer.add(
    ...
    private fun highlightListBlock(listBlock: ListBlock) {
        writer.add(
            spanPool.leadingMargin(listBlockIndentationMargin),
            listBlock.startOffset,
            listBlock.endOffset
        )
    }
    ...
    private fun highlightListItem(listItem: ListItem) {
        writer.add(
            spanPool.foregroundColor(syntaxColor),
            listItem.startOffset,
            listItem.startOffset + 1
        )
    }
    ...
    private fun highlightThematicBreak(thematicBreak: ThematicBreak) {
        ...
        if (canDrawHorizontalRule) {
            ...
            val hrSpan = spanPool.horizontalRule(
                immutableThematicBreakChars,
                horizontalRuleColor,
        ...
        writer.add(
            spanPool.foregroundColor(syntaxColor),
            thematicBreak.startOffset,
            thematicBreak.endOffset
        )
    }
