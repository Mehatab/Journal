...
interface NoteDao {
    ...
    @Query("SELECT n.*, nb.color as color from Note n JOIN Notebook nb ON n.notebook_id = nb.notebook_id WHERE (n.note LIKE :query)")
    ...
    @Query("SELECT n.* from Note n where n.notebook_id =:id AND (n.note LIKE :q)")
